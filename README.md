...
- In exactly the same way, it does the same to human language, but humans also have to ACT the same as the way their language aligns. So this is more complicated. Sanctuary System is about how to get intelligences to do that by first getting them to make these OMNISANC AIs (SANCREVTWILITELANG agents) and so on, and then to use those intelligences to finish it over time by continuing indefinitely. It leverages the allegories to create a kalipolis-type situation where "all roads lead to Rome", where Rome is the realization of the way the system is engineered and why.
